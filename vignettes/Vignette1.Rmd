...
The functionality of COMA is illustrated using tetraploid potato data from the University of Wisconsin-Madison. 

The starting point for analysis is the function `read_data`. The first argument is the name of the CSV input file with the marker effects and allele dosage data. The second argument is the name of the CSV input file with the kinship matrix. If the kinship file is omitted, `read_data` expects an additional column in the geno file with base population allele frequencies, to properly calculate inbreeding. Consult the help page for more information on file formats. 

The `matings` argument indicates how to create the table of possible matings. The option below is "all", which creates all unique parental combinations. When argument `standardize` is TRUE, the trait is centered and scaled so that the parental population has zero mean and unit variance. This is for convenience when interpreting the response values.
```{r}
library(COMA)
geno.file <- system.file("vignette_data", "geno.csv", package = "COMA")
kinship.file <- system.file("vignette_data", "kinship.csv", package = "COMA")

#preview files
geno <- read.csv(geno.file,check.names=F)
geno[1:4,1:5]
kinship <- read.csv(kinship.file,check.names=F,row.names=1)
kinship[1:3,1:3]
...
As shown above, the `parents` output contains the predicted merit (GEBV) for each individual. The `matings` output contains the predicted merit ( F1 progeny mean) for each possible mating.

The `ocs` and `oma` functions are used to determine the optimal genetic contributions. Both require the argument `parents`, which in addition to the merits must have the minimum and maximum contributions. In this case we used min=0 and max=1 for all parents, but this can be tailored to each parent. The `oma` function also requires a table of possible matings with merit, min, and max.
Common guidance in animal breeding is to target an inbreeding rate (dF) of 0.005 to 0.01, so we begin with those values.
...
oma2b <- oma(dF=0.01, parents=parents, matings=matings, ploidy=4, 
...
```{r}
dF <- c(0.5,1,2,3)/100
ans <- lapply(as.list(dF), oma, parents=parents, matings=matings, 
              ploidy=4, K=data$K, min.a=0.005)
response <- t(sapply(ans,"[[",1))
kable(response, digits=3)
```

The above table illustrates the significance of the dF2 parameter, which is hitting the upper limit even when dF1 is not.
...
```
The virus resistance trait is important enough that we typically require every cross to have at least one resistant parent. Furthermore, CMS individuals cannot be male parents. We will therefore remove any matings that violate these requirements (equivalent to putting the maximum allocation at 0).

```{r}
CMS.test <- !(matings$male %in% CMS.id)
PVY.test <- matings$female %in% PVYR.id | 
              matings$male %in% PVYR.id
matings2 <- matings[CMS.test & PVY.test,]
nrow(matings)
nrow(matings2) #eliminates 2168 possible matings

oma2c <- oma(dF=0.01, parents=parents, matings=matings2, ploidy=4, 
             K=data$K, min.a=0.005)
oma2c$response
```

Compared to the previous results, the optimum response is lower under this restricted mating plan, which is to be expected. 

COMA also accepts linear equality and inequality constraints on the parental contributions. For example, even though we have ensured no CMS clones are male parents, if all females were CMS, the next generation would have no potential male parents! We therefore need to limit the parental contribution of CMS clones. The following code shows that the total contribution for the previous solution was 38.6%.
...
Let's try limiting the total contribution to 20%. This is accomplished by adding an additional column to the `parents` data.frame, where the values represent the coefficients $\mathbf{a}$ of the contribution variables $\mathbf{y}$, and the column header is parsed to complete the equation. In this case, the header "lt0.5" represents less than or equal to 0.2:
$$ \mathbf{a}^\prime \mathbf{y} \le 0.2$$

```{r}
parents$lt0.2 <- ifelse(parents$id %in% CMS.id,1,0)
head(parents)

oma2d <- oma(dF=0.01, parents=parents, matings=matings2, ploidy=4, 
             K=data$K, min.a=0.005)
oma2d$response
